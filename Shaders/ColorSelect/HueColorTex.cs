...
namespace RedPaint
{
    public class HueColorTex : AbstrShaderTex
    {
        ...
        {
            Generate(sizeX, sizeY, baseColor);
        }

        public void Generate(int width, int height, Color baseColor)
        {
            Dispose();
            ...
            if (Effect == null)
            {
                Effect = mc.Content.Load<Effect>("Shaders/HueRect");
            }
            ...
            Effect.Parameters["Color"].SetValue(baseColor.ToVector4());
        ...
        public HueColorTex(Maincode imc) : base(imc)
        {
